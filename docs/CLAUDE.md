...
|---------------------------------------------|-----------|---------|
| What Is EveryStack                          | 36–44     | Multi-tenant SaaS unifying no-code databases; target user, core differentiator (cross-base linking) |
| Tech Stack                                  | 46–71     | Full technology table: Next.js, TypeScript, Drizzle, PostgreSQL, Redis, Socket.io, Clerk, Cloudflare R2, Anthropic Claude |
| Monorepo Structure                          | 73–121    | Directory tree for apps/web, apps/worker, apps/realtime, packages/shared, docs/ |
| Key Commands                                | 123–142   | pnpm/turbo CLI commands for dev, build, lint, typecheck, test, migrate, seed |
| Architecture Fundamentals                   | 144–193   | Canonical JSONB pattern, multi-tenant RLS, AIService abstraction, FieldTypeRegistry, permission model, entity hierarchy |
| Platform Authentication & Onboarding        | 195–235   | Clerk auth provider, signup + onboarding wizard, team invitation flow, JWT session handling |
| Code Conventions                            | 237–277   | Naming (kebab/camel/Pascal/snake), async/await + Zod + Drizzle patterns, shadcn/ui components, TypeScript strict rules |
| Testing Rules                               | 279–313   | File naming conventions, 6 non-negotiable rules (tenant isolation, factories, coverage), enforced coverage targets |
| Error Handling — Default Patterns           | 315–355   | AppError shape, per-surface error contracts, default UI error patterns, logging defaults |
| Critical Rules — Do's and Don'ts            | 357–385   | 10 DOs (glossary, FieldTypeRegistry, tenant routing) and 10 DON'Ts (no raw SQL, no any, no post-MVP) |
| Design Philosophy                           | 387–403   | Progressive disclosure (3 levels), three creation flow patterns (inline, wizard, recipe) |
| Documentation Hierarchy (3-Tier)            | 405–451   | Root CLAUDE.md (always loaded), directory CLAUDE.md (contextual), reference docs (on-demand); key docs by domain |
| State Files                                 | 453–509   | DECISIONS.md (tactical log), MODIFICATIONS.md (per-session changelog), TASK-STATUS.md (unit checklist); append-only rules |
| MVP Scope Guard                             | 511–521   | MVP inclusion/exclusion list; build extension points only, not extensions |
| Reference Doc vs. Phase Build Doc Hierarchy | 523–531   | Authority split: reference docs for architecture/schema, phase build docs for sequencing |
| Agent Roster                                | 533–543   | 5 agents (Architect, Planner, Builder, Reviewer, Docs) with lifecycle positions and branch prefixes |
| Build Lifecycle — Steps & Planning Gates    | 545–655   | Steps 0–5 + Gates 1–4: doc prep, subdivision, playbook gen, build, review, docs sync; RSA classifications (D/SH/PJ) |
| Scope Label Discipline                      | 657–668   | MVP and Post-MVP scope label vocabulary; never use phase numbers in reference docs |
| CockroachDB Readiness — Active Safeguards   | 670–680   | 5 active safeguards: UUIDv7, no PG-specific syntax, explicit transactions, no advisory locks, hash-partition-compatible |
| Pre-Merge Gates (CI)                        | 682–694   | 9 CI gates: TypeScript, ESLint, i18n, tests, coverage, migration timing, AI eval |
...
Directory layout for apps/web (Next.js), apps/worker (BullMQ), apps/realtime (Socket.io), packages/shared (db, sync, ai, testing), and docs/.
All shared packages are imported as `@everystack/*` workspace packages.
...
Five foundational patterns: canonical JSONB data flow, multi-tenant RLS via `getDbForTenant()`, AIService capability-tier abstraction, FieldTypeRegistry for field transforms, and the 5-role two-layer permission model.
Entity hierarchy: tenant → boards → workspaces → tables → views/records/fields. See `data-model.md`, `permissions.md`, `ai-architecture.md`.
...
Clerk-based auth for platform users (not portal clients), signup + 3-step onboarding wizard, team member invitation flow, and JWT session handling.
Portal routes use a separate auth system — see `portals.md`.
...
Naming rules (files, variables, types, DB columns, constants, JSONB keys), async/Zod/Drizzle patterns, shadcn/ui + Tailwind component conventions, and TypeScript strict-mode enforcement.
Thread table column types (`author_type`, `message_type`, `scope_type`) are VARCHAR — see inline rule.
...
Covers test file naming, 6 non-negotiable rules (tenant isolation via `testTenantIsolation()`, factory usage, coverage), and per-package coverage thresholds.
See `testing.md` for full test strategy, Vitest/Playwright configuration, and CI pipeline details.
...
Defines the 3-tier doc loading model: root CLAUDE.md (always), directory CLAUDE.md (contextual), `docs/reference/*.md` (on-demand).
Maps domains to reference docs — data-model.md, sync-engine.md, permissions.md, design-system.md, ai-architecture.md, etc.
...
Defines the 6-step agent workflow (Steps 0–5) and 4 planning gates connecting Architect, Planner, Builder, Reviewer, and Docs agents.
Covers RSA classifications (D/SH/PJ), context budget tests, subdivision planning, and failure replanning. See `SUBDIVISION-STRATEGY.md`, agent skill docs.
