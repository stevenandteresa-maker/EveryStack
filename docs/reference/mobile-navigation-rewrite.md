...
- **Board View *(post-MVP)*:** "+" at the bottom of each column.
- **Calendar View *(post-MVP)*:** Tap empty time slot → new record with date pre-filled.
...
| *(Phase build doc — Core UX)* | When creating a Core UX phase build doc, fold "Table-scoped filter bar search" into Command Bar scope behavior rather than listing it separately. |
