...
- **Comment Threads:** Record-level comments using two-thread model (internal thread for workspace users, client thread for portal clients — `thread_type` discriminator). Uses `threads` / `thread_messages` model.
...
| `slug`           | VARCHAR                | URL slug. UNIQUE(tenant_id, slug) — tenant-scoped per CP-001-A. URL: `portal.everystack.app/{tenant-slug}/{portal-slug}`.                                                                                                                                                                                                                                            |
