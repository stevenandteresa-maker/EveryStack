 ...
 *
 * Handles the redirect from Airtable after the user authorizes.
 * Completes the OAuth flow and notifies the opener window via postMessage.
 *
 * Flow:
 * 1. Read `code` and `state` from query params
 * 2. Call `completeAirtableConnection` to exchange code for tokens
...
import { completeAirtableConnection } from '@/actions/sync-connections';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const { connectionId } = await completeAirtableConnection({ code, state });
...
<script>
(function() {
  var data = { type: 'airtable_oauth_error', error: 'Connection failed. Please try again.' };
  var origin = ${JSON.stringify(appUrl)};
